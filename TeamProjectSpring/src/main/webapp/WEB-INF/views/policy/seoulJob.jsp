...
<%@ page import="com.dlt.senior.policy.dao.SWelfareRepository"%>
<%@ page import="com.dlt.senior.policy.model.PolicyVO"%>
<%@ page import="java.util.List" %>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
					<%
						int regionId = Integer.parseInt(request.getParameter("region_id"));
						List<PolicyVO> pub = (ArrayList<PolicyVO>) request.getAttribute("publicDtos");
					%>
<!DOCTYPE html>
<html>
...
<title>DLT_일자리</title>
<link rel="stylesheet" href="${pageContext.request.contextPath}/css/policyJob.css">
		...
		<div class="container">
			<div class="side-menu">
				<div>
					<div class="text2">
						<% if(regionId == 1){ %>
						<a href="${pageContext.request.contextPath}/">홈</a> > <a href="${pageContext.request.contextPath}/policy">정책</a> > 서울 > 어르신  일자리
						<% } %>
						<% if(regionId == 2){ %>
						<a href="${pageContext.request.contextPath}/">홈</a> > <a href="${pageContext.request.contextPath}/policy">정책</a> > 전남 > 어르신 일자리
						<% } %>
					</div>
				</div>
				...
				<div class="policyButton">
						...
						onclick="location.href='<c:url value='/policy/job?region_id=1' />';">어르신 일자리</button>
						<% } %>
						<% if(regionId == 2){ %>
						<button id="current" class="policyBtn" type="button"
						onclick="location.href='<c:url value='/policy/job?region_id=2' />';">어르신 일자리</button>
						<% } %>
						
						<% if(regionId == 1){ %>
							<button class="policyBtn" type="button"
							onclick="location.href='<c:url value='/policy/social?region_id=1' />';">여가 복지 지원</button>
						<% } %>
						
						<% if(regionId == 2){ %>
							<button class="policyBtn" type="button"
							onclick="location.href='<c:url value='/policy/social?region_id=2' />';">여가 복지 지원</button>
						<% } %> 
						
						<% if(regionId == 1){ %>
								<button class="policyBtn" type="button"
								onclick="location.href='<c:url value='/policy/life?region_id=1' />';">생활 안정 지원</button>
						<% } %>
						<% if(regionId == 2){ %>
								<button class="policyBtn" type="button"
								onclick="location.href='<c:url value='/policy/life?region_id=2' />';">생활 안정 지원</button>
						<% } %>
						<% if(regionId == 1){ %>
									<button class="policyBtn" type="button"
									onclick="location.href='<c:url value='/policy/health?region_id=1' />';">어르신 건강 증진</button>
						<% } %>
						<% if(regionId == 2){ %>
									<button class="policyBtn" type="button"
									onclick="location.href='<c:url value='/policy/health?region_id=2' />';">어르신 건강 증진</button>
						<% } %>
			...
			<div>
				...
				<div class="as_seoultable">
					...
					<%
					List<PolicyVO> Social = (ArrayList<PolicyVO>) request.getAttribute("socialDtos");
									int cnt = 1;
					%>
					...
					<%
					List<PolicyVO> market = (ArrayList<PolicyVO>) request.getAttribute("marketDtos");
					%>
					<div>
